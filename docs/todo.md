...
- [ ] Build the three-vrm render parity harness.
  ...
  - `just render-parity-real-transparent` covers the same six real external fixtures on transparent background. Current alpha mismatches are all small and below tolerance `64`: Seed-san wgpu/Bevy `25/32`, constraint `11/11`, UV animation `0/0`, expression mask samples `12/12`, Alicia VRM0 `32/32`. Current selected `rgb-visible` PSNR remains a regression floor rather than final compatibility: Seed-san wgpu `20.4130 dB`, Bevy `20.3065 dB`; constraint wgpu `25.6531 dB`, Bevy `25.6476 dB`; UV animation wgpu `27.4437 dB`, Bevy `27.4271 dB`; expression masks around `29.84 dB`; Alicia wgpu `20.3469 dB`, Bevy `20.3434 dB`.
  ...
  - Remaining parity blockers: exact MToon light/color accumulation and runtime/material breadth on broader real model fixtures beyond the covered reference-shaped accumulator, generated light fixture, and v0 shade clamp; raising real transparent-fixture PSNR beyond the current low regression floor; broader real-fixture screen-coordinate outline discovery beyond the generated fixture; and higher PSNR thresholds.
