...
- [ ] Build the three-vrm render parity harness.
  ...
  - `just render-parity-real-normal-maps` now keeps a focused review directory for the unique real tangentless normal-map fixtures reported by `inspect-mtoon-fixtures`: Seed-san has `6` normal-mapped primitives without tangents and `VRM1_Constraint_Twist_Sample.vrm` has `3`. The current run enforces selected `rgb-visible >= 32.5 dB` with exact alpha parity: Seed-san wgpu `32.8751 dB` / Bevy `32.5273 dB`, and constraint sample wgpu `35.9456 dB` / Bevy `35.9394 dB`.
  - `just render-parity-outline-off` now disables MToon outline expansion across three-vrm, wgpu, and Bevy for diagnosis. Seed-san remains almost unchanged at wgpu `32.5179 dB` / Bevy `32.5257 dB`, while the constraint sample improves to wgpu `36.8185 dB` / Bevy `36.8061 dB`, so outline expansion is a concrete contributor for constraint but not the main Seed-san blocker.
  ...
  - A source-like generated MToon light/color fixture is available through `just render-parity-mtoon-light-generated`. It covers forced base, forced shade, three-vrm-compatible ignored MToon `occlusionTexture`, parametric rim, matcap rim, mixed rim/matcap, angled-normal toon-ramp quads, pure `emissiveFactor`, and `emissiveTexture` multiplied by `KHR_materials_emissive_strength` without committing binary assets. The recipe uses transparent background plus `rgb-interior1px` so the selected score measures swatch interior color rather than the known raster edge delta; current selected PSNR is wgpu `58.9968 dB` and Bevy `51.8032 dB`, with max selected channel deltas `1` and `2`. It also writes per-swatch color reports with `tools/render-parity/compare-swatch-colors.rs`, currently enforcing every wgpu swatch at max channel delta `<= 1` and every Bevy swatch at max channel delta `<= 2`. `just render-parity-mtoon-light-direct-generated` disables ambient on both sides and still passes at wgpu `58.9803 dB` / Bevy `51.7674 dB`, keeping direct-light color parity separate from the default ambient `0.1` reference setup. `just render-parity-mtoon-light-colored-generated` adds a non-white direct-light guard (`1.0, 0.55, 0.25`) and passes at wgpu `58.8377 dB` / Bevy `52.0812 dB`, proving direct diffuse and rim-light color accumulation are not only matching for white lights. `just render-parity-mtoon-light-ambient-generated` disables directional light on both sides through the three-vrm reference intensity and Rust direct-light scale controls, and passes at wgpu `60.9964 dB` / Bevy `53.7214 dB`. All four light recipes now enforce aggregate selected-channel delta `<= 2` and run the same swatch guard, so exact direct/colored-direct/ambient accumulation regressions are separated by report directory.
  ...
  - Remaining parity blockers: runtime/material breadth on broader real model fixtures beyond the covered reference-shaped accumulator, improving real outline expansion for the constraint sample, identifying Seed-san's remaining non-outline material/geometry delta, raising real tangentless normal-map thresholds beyond the current `32.5 dB` real-fixture floor, real transparent-fixture visual parity beyond the current regression floors, finding or legally adding a real screen-coordinate outline fixture if one becomes available beyond the generated guard, and higher PSNR thresholds.
