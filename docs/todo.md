...
- [ ] Build the three-vrm render parity harness.
  ...
  - Remaining parity blockers: runtime/material breadth on broader real model fixtures beyond the covered reference-shaped accumulator, raising real tangentless normal-map thresholds beyond the current `32 dB` real-fixture floor, real transparent-fixture visual parity beyond the current regression floors, finding or legally adding a real screen-coordinate outline fixture if one becomes available beyond the generated guard, and higher PSNR thresholds.
