...
- [ ] Build the three-vrm render parity harness.
  ...
  - A license-safe generated transparent fixture is available through `just render-parity-transparent-generated`. It writes a source-like VRM1 glTF under `.external-fixtures/generated/`, renders on a transparent background, and currently verifies exact alpha-bucket parity for overlapping MToon `BLEND` primitives (`transparent=512 opaque=0 partial=65024`, alpha mismatches `0`) across three-vrm, wgpu, and Bevy. It also verifies transparent RGB accumulation: wgpu is byte-exact against three-vrm on the selected `rgb-visible` metric, and Bevy is within max channel delta `1`.
