...
- [ ] Build the three-vrm render parity harness.
  ...
  - The first broader official-sample render sweep covers `Seed-san.vrm` and `VRM1_Constraint_Twist_Sample.vrm`. Constraint sample PSNR is now wgpu `34.33 dB` and Bevy `34.33 dB`; both still miss the MToon-lit threshold but prove the harness is no longer single-fixture only.
