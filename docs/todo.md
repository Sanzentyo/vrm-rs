...
- [ ] Build the three-vrm render parity harness.
  ...
  - Bevy capture now has a Bevy 0.18.1 headless offscreen example using real mesh and texture inputs plus shared RGBA/PNG output. The Seed-san PSNR baseline is `24.07 dB` after rest-skinning, MToon-derived alpha/cull/render-order policy, matching the three-vrm/wgpu 30 degree camera projection, baking an approximate MToon light response into vertex colors, and adding expanded outline meshes. The wgpu baseline is `25.89 dB` after matching three.js directional-light vector convention and adding expanded outline draws.
  - Remaining parity blockers: Bevy MToon shading/runtime state, wgpu expression/runtime state, exact MToon light accumulation, secondary shade/matcap/rim/normal texture paths, outline width texture/screen-coordinate details, and higher PSNR thresholds.
