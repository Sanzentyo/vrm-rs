...
- [ ] Build the three-vrm render parity harness.
  ...
  - A source-like generated expression morph fixture is available through `just render-parity-morph-expression-generated`. It passes `happy=1.0` through three-vrm's `expressionManager`, wgpu, and Bevy, covers non-zero morph target expression writeback in the concrete render captures, and currently reports selected `rgb-interior1px` PSNR `Infinity` for both Rust renderers with max selected channel delta `0`; the known edge-only alpha mismatch is `3` pixels with tolerance `8`.
