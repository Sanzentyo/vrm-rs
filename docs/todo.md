...
- [ ] Build the three-vrm render parity harness.
  ...
  - Bevy capture now has a Bevy 0.18.1 headless offscreen example using real mesh and texture inputs plus shared RGBA/PNG output. The Seed-san PSNR baseline is `23.77 dB` after rest-skinning, MToon-derived alpha/cull/render-order policy, matching the three-vrm/wgpu 30 degree camera projection, and baking an approximate MToon light response into vertex colors. The wgpu baseline is `24.32 dB` after matching three.js directional-light vector convention.
