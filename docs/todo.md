...
- [ ] Build the three-vrm render parity harness.
  ...
  - A source-like generated MToon texture-slot fixture is available through `just render-parity-mtoon-textures-generated`. It covers `shadingShiftTexture`, `rimMultiplyTexture`, `uvAnimationMaskTexture`, and `outlineWidthMultiplyTexture` at `mtoon-time=1.0` with exact alpha parity and selected `rgb-interior1px` PSNR wgpu `53.3872 dB` / Bevy `50.9296 dB`; normal-map breadth remains real-fixture-only because the three-vrm WebGL reference rejects the synthetic normal-map material with a `vTangent` validation error.
  ...
  - `tools/render-parity/inspect-mtoon-fixtures.rs` and `just inspect-mtoon-fixtures` now inventory MToon feature coverage from local fixtures without committing binary samples. The current official fixture set contains no screen-coordinate outline materials, but does cover world outline, transparent-ZWrite, UV animation, normal textures, matcap textures, and shade textures; generated fixtures provide the missing screen-outline, strict transparent-ZWrite, and texture-slot guards. The scanner also shows `21` official normal-mapped primitives without glTF `TANGENT`, making three-vrm's derivative normal-map fallback a concrete follow-up candidate; a naive derivative attempt was measured and rejected after Seed-san fell to `20.3102 dB`.
  - Remaining parity blockers: runtime/material breadth on broader real model fixtures beyond the covered reference-shaped accumulator, angled-normal generated light fixture, MToon occlusion-ignore guard, and v0 shade clamp; normal-map parity for tangentless official primitives; raising real transparent-fixture PSNR and visual parity beyond the current `32 dB` floor; finding or legally adding a real screen-coordinate outline fixture if one becomes available beyond the generated guard; and higher PSNR thresholds.
