...
- [ ] Build the three-vrm render parity harness.
  ...
  - Bevy capture now has a Bevy 0.18.1 headless offscreen example using real mesh and texture inputs plus shared RGBA/PNG output. The Seed-san PSNR baseline is `24.98 dB` after rest-skinning, MToon-derived alpha/cull/render-order policy, matching the three-vrm/wgpu 30 degree camera projection, baking an approximate MToon light response into vertex colors, adding expanded outline meshes with outline width texture sampling, and applying the measured reference-exposure correction. The wgpu baseline is `27.52 dB` after matching three.js directional-light vector convention, adding expanded outline draws with outline width texture sampling, applying the same correction, and binding MToon shade/matcap textures separately from the main texture.
  - Remaining parity blockers: Bevy MToon shading/runtime state, wgpu expression/runtime state, exact MToon light accumulation, rim/normal texture paths, screen-coordinate outline details, and higher PSNR thresholds.
