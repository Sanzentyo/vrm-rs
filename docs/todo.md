...
- [ ] Build the three-vrm render parity harness.
  ...
  - `tools/ci/local-ci.rs --render-parity` now regenerates three-vrm, wgpu, and Bevy RGBA artifacts under the same canonical transparent background contract, verifies that wgpu/Bevy alpha masks match the three-vrm reference within `--render-alpha-mismatch-tolerance`, writes canonical PNGs encoded from those RGBA artifacts, PSNR reports, amplified RGB/alpha diff heatmaps, and `.external-fixtures/render-parity/visual-review.html` in one local command. It defaults to `Seed-san.vrm` and accepts repeated `--render-fixture NAME.vrm` flags for broader official-sample sweeps. Direct `three-vrm-browser-capture.mjs --png-out` now also encodes PNG from raw RGBA readback data, so the standalone three-vrm preview path no longer depends on browser canvas compositing.
  ...
  - The broader official-sample render sweep covers `Seed-san.vrm`, `VRM1_Constraint_Twist_Sample.vrm`, and `VRMC_materials_mtoon_UV_Animation_Test.vrm`; a separate `just render-parity-uv-animation` recipe advances MToon material time to `1.0` for UV scroll/rotation parity. Current transparent-background PSNR is Seed-san wgpu `28.42 dB`, Seed-san Bevy `28.25 dB`, constraint sample wgpu/Bevy `34.33 dB`, UV animation time `0` wgpu/Bevy `36.16 dB`, and UV animation time `1.0` wgpu/Bevy `35.92 dB`.
  - Remaining parity blockers: exact MToon light/color accumulation, screen-coordinate outline fixture discovery/measurement, broader mask/transparent material fixture render parity, and higher PSNR thresholds.
