...
- [ ] Build the three-vrm render parity harness.
  ...
  - `just render-parity-real-normal-maps` now keeps a focused review directory for the unique real tangentless normal-map fixtures reported by `inspect-mtoon-fixtures`: Seed-san has `6` normal-mapped primitives without tangents and `VRM1_Constraint_Twist_Sample.vrm` has `3`. The current run enforces selected `rgb-visible >= 32 dB` with exact alpha parity: Seed-san wgpu `32.8751 dB` / Bevy `32.5273 dB`, and constraint sample wgpu `35.9456 dB` / Bevy `35.9394 dB`.
  ...
  - Remaining parity blockers: runtime/material breadth on broader real model fixtures beyond the covered reference-shaped accumulator, raising real tangentless normal-map thresholds beyond the current `32 dB` real-fixture floor, improving real outline expansion for the constraint sample, identifying Seed-san's remaining non-outline material/geometry delta, real transparent-fixture visual parity beyond the current regression floors, finding or legally adding a real screen-coordinate outline fixture if one becomes available beyond the generated guard, and higher PSNR thresholds.
