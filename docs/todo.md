...
- [ ] Build the three-vrm render parity harness.
  ...
  - `tools/render-parity/inspect-mtoon-fixtures.rs` and `just inspect-mtoon-fixtures` now inventory MToon feature coverage from local fixtures without committing binary samples. The current official fixture set contains no screen-coordinate outline materials, but does cover world outline, transparent-ZWrite, UV animation, normal textures, matcap textures, and shade textures; generated fixtures provide the missing screen-outline and strict transparent-ZWrite guards.
