...
- [ ] Build the three-vrm render parity harness.
  ...
  - A license-safe generated transparent fixture is available through `just render-parity-transparent-generated`. It writes a source-like VRM1 glTF under `.external-fixtures/generated/`, renders on a transparent background, and currently verifies exact alpha-bucket parity for overlapping MToon `BLEND` primitives (`transparent=512 opaque=0 partial=65024`, alpha mismatches `0`) across three-vrm, wgpu, and Bevy. It also verifies transparent RGB accumulation with an embedded bufferView PNG base-color texture: wgpu reports selected `rgb-visible = 53.0238 dB` with max channel delta `1`, and Bevy reports `49.7151 dB` with max channel delta `2`. The fixture includes `COLOR_0` to guard the spec/three-vrm behavior that MToon ignores vertex colors while renderer IO still extracts them for non-MToon materials.
  ...
  - Remaining parity blockers: exact MToon light/color accumulation beyond the covered v0 shade clamp, broader real-fixture transparent material coverage beyond the generated blend fixture, high-contrast Bevy transparent layer ordering, screen-coordinate outline fixture discovery/measurement, and higher PSNR thresholds.
