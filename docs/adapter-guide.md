...
For Bevy, `vrm-adapter-bevy` starts with a 0.18.1 registry/descriptor/runtime skeleton. `BevyNodeMap` maps `NodeRef` to entities, `bevy_mtoon_descriptors` bridges renderer-agnostic MToon descriptors, and `VrmRuntimePlugin` installs `BevyVrmRuntimeConfig` as the first ECS entry point for runtime policy. Concrete Bevy transform, morph, material, and mesh writeback systems are still downstream work. For custom wgpu/ash engines, keep renderer resources outside `vrm-rs` and use `MaterialRef`/`TextureRef` as stable lookup keys into engine-owned tables.
