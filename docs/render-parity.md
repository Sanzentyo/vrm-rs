...
- `just render-parity-acceptance-import-bundle <bundle-or-zip> <label>`:
  validate and copy one returned strict bundle directory or `.zip` into
  ...
  intended external-fixture root. Zip imports are safely unpacked into `target/`
  first and may include a wrapper directory around `acceptance-bundle/`. Pass
  `replace=true` only when deliberately refreshing an existing runner label.
...
the visual notes. The older one-command
...
`just render-parity-acceptance-import-bundle <returned-bundle-path-or-zip>
<label>`, or added manually as another sibling directory under
...
for `49/49` shared owner pixels. The context run uses `--context-radius 3`,
emits `109` source-neighborhood triangles, and passes under
...
`--render-fail-under 34`. Use
`just render-parity-real-normal-maps-ash-gated` when the Ash readback path
should be checked by the same gate. The current Ash-gated run writes
`.external-fixtures/render-parity-real-normal-maps-ash-gated/` and reports
Seed-san wgpu `34.6538 dB`, Bevy `34.1163 dB`, Ash `34.6391 dB`, constraint
wgpu `36.2518 dB`, Bevy `36.2349 dB`, and Ash `36.2509 dB`, all with alpha
mismatches `0`. The Bevy capture fills missing generated tangent vertices with
`vrm-io::fallback_tangent`, so one degenerate vertex no longer disables normal
maps for the whole primitive. Review the normal-map visual-review HTML and
