...
- Adapter spring parity uses `SpringRestMap` plus `CenterSpringRuntimeState` so initial local child positions, center-space tail state, sparse spring-chain children, first scene-child fallback, and VRM0 final-joint fallback are captured separately from mutable frame state. The parity path reads the selected child's local translation to match three-vrm's `child.position`, and high-level parity driver ticks require a `WorldTransformUpdate` synchronization point before spring simulation.
