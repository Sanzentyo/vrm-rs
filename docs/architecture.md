...
- `vrm-vmc`: typed VMC message conversion, sender/rate/time policy gating, and
  ...
  authentication sources, and networking retries stay in applications or engine
  integrations.
...
- MToon exposes renderer hints such as render order and outline enablement, plus a renderer-neutral GPU ABI (`MtoonGpuUniform`, `MtoonGpuMaterial`, binding helpers, `MtoonLitShadeInputs`, and `MTOON_REFERENCE_WGSL`) for engines that want shared parameter packing. The public reference helpers use the same three-vrm-shaped linearstep toon-ramp contract as the concrete wgpu, Bevy, and Ash capture shaders. Backend-specific shader modules, pipeline objects, descriptor allocation, and draw submission remain outside `vrm-core`.
