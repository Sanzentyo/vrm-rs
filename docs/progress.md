...
- Added `tools/three-vrm-golden.mjs` to generate ignored three-vrm spring golden JSON from real VRM files, plus an ignored adapter comparison test for stable-length Seed-san spring joint rotations.
...
- Spring bone parity now has stable-length three-vrm golden comparison on Seed-san, but tiny-tail joints (`<= 0.001`) still need a dedicated numeric strategy.
