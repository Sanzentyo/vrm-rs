...
struct Options {
    ...
    #[arg(long)]
    render_force_nearest_textures: bool,
    ...
    render_owner_id_color_source: RenderOwnerIdColorSource,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, ValueEnum)]
enum RenderOwnerIdPhaseOrderPolicy {
    Full,
    Off,
    OverlapArea,
    OverlapTriangle,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, ValueEnum)]
enum RenderOwnerIdColorSource {
    VertexColor,
    Uniform,
}

impl RenderOwnerIdColorSource {
    fn as_cli_value(self) -> &'static str {
        match self {
...
fn run_render_tool_self_tests() -> Result<(), String> {
    ...
    run_cmd(
        ...
        [
            "+nightly",
            "-Zscript",
            "tools/render-parity/audit-owner-sample-execution.rs",
            "--self-test",
        ],
    )?;
    run_cmd(
        ...
        [
            "+nightly",
            "-Zscript",
            "tools/render-parity/audit-texture-sampling-parity.rs",
            "--self-test",
        ],
    )?;
    run_cmd(
    ...
    run_cmd(
        ...
        [
            "+nightly",
            "-Zscript",
            "tools/render-parity/validate-current-acceptance-bundle.rs",
            "--self-test",
        ],
    )?;
    run_cmd(
    ...
    run_cmd(
        ...
        [
            "+nightly",
            "-Zscript",
            "tools/render-parity/validate-acceptance-repeat.rs",
            "--self-test",
        ],
    )?;
    run_cmd(
...
fn capture_three_vrm_reference(options: &Options, fixture: &RenderFixture) -> Result<(), String> {
    ...
    if options.render_force_nearest_textures {
        command.arg("--force-nearest-textures");
    }
...
fn capture_wgpu(options: &Options, fixture: &RenderFixture) -> Result<(), String> {
    ...
    command
        ...
        .arg(options.render_diagnostic_mode.as_cli_value())
        .arg("--front-face")
        .arg(options.render_front_face.as_cli_value());
    if options.render_disable_outlines {
        command.arg("--disable-outlines");
    }
    command
...
fn capture_bevy(options: &Options, fixture: &RenderFixture) -> Result<(), String> {
    ...
    command
        ...
        .arg("--diagnostic-render")
        .arg(options.render_diagnostic_mode.as_cli_value())
        .arg("--owner-id-phase-order-policy")
        .arg(options.render_owner_id_phase_order_policy.as_cli_value())
        .arg("--owner-id-color-source")
        .arg(options.render_owner_id_color_source.as_cli_value())
        .arg("--front-face")
        .arg(options.render_front_face.as_cli_value());
    if options.render_disable_outlines {
        command.arg("--disable-outlines");
...
fn capture_ash_readback(
    ...
    command
        ...
        .arg(options.render_camera_z.to_string())
        .arg("--direct-light-scale")
        .arg(render_light_units(options).direct_light_scale.to_string())
        .arg("--directional-r")
        .arg(options.render_directional_r.to_string())
        .arg("--directional-g")
        .arg(options.render_directional_g.to_string())
...
fn render_summary_markdown(
    ...
    let meta = RenderSummaryMeta {
        ...
        owner_id_phase_order_policy: options
            .render_owner_id_phase_order_policy
            .as_cli_value()
            .to_owned(),
        owner_id_color_source: options
            .render_owner_id_color_source
            .as_cli_value()
            .to_owned(),
        front_face: options.render_front_face.as_cli_value().to_owned(),
        ...
        force_nearest_textures: options.render_force_nearest_textures,
...
fn render_review_manifest_value(
    ...
    Ok(serde_json::json!({
        ...
        "forceNearestTextures": options.render_force_nearest_textures,
...
struct RenderSummaryMeta {
    ...
    owner_id_phase_order_policy: String,
    owner_id_color_source: String,
    front_face: String,
...
fn render_summary_markdown_from_rows(
    ...
    output.push_str(&format!(
        ...
        meta.owner_id_phase_order_policy,
        meta.owner_id_color_source,
        meta.front_face,
        meta.normal_map_mode,
        meta.normal_map_scale,
        meta.disable_texture_mips,
        meta.force_nearest_textures,
