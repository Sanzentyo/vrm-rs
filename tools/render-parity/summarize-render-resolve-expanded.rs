...
fn render_markdown(summary: &ExpandedSummary) -> String {
            ...
            for row in &join.pbr_term_outputs {
                out.push_str(&format!(
                    ...
                    fmt_projection_distances(
                        row.mean_browser_total_srgb_actual_distance,
                        row.mean_browser_total_srgb_expected_distance,
                        row.browser_total_srgb_sample_count,
                    ),
                    fmt_closer_counts(
                        row.browser_total_srgb_actual_closer,
                    ...
                    fmt_fit_pair(
                        row.browser_actual_direct_scale_rgb,
                        row.browser_actual_ambient_scale_rgb,
                        row.browser_light_fit_sample_count,
                        row.browser_light_fit_min_normalized_determinant,
                    ),
                    fmt_fit_pair(
                        row.browser_expected_direct_scale_rgb,
                        row.browser_expected_ambient_scale_rgb,
                        row.browser_light_fit_sample_count,
                        row.browser_light_fit_min_normalized_determinant,
                    ),
                    fmt_projection_distances(
                        ...
                        row.rust_total_srgb_sample_count,
                    ),
                    fmt_closer_counts(
                    ...
                    fmt_fit_pair(
                        row.rust_actual_direct_scale_rgb,
                        row.rust_actual_ambient_scale_rgb,
                        row.rust_light_fit_sample_count,
                        row.rust_light_fit_min_normalized_determinant,
                    ),
                    fmt_fit_pair(
                        row.rust_expected_direct_scale_rgb,
                        row.rust_expected_ambient_scale_rgb,
                        row.rust_light_fit_sample_count,
                        row.rust_light_fit_min_normalized_determinant,
                    ),
    ...
    if !summary.texture_audits.is_empty() {
        ...
        for audit in &summary.texture_audits {
            ...
            out.push_str("| Material | Draw key | Count | Classification | Action | Mean E-A | Manifest A/E | A-M RGB | E-M RGB | LS gain A/M | LS gain E/M | Missing LS gain (E-A)/M | Near sample A/E/Both-far |\n");
            out.push_str("| --- | --- | ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n");
            for probe in &audit.recommended_probes {
                out.push_str(&format!(
                    ...
                    fmt_optional_vec3(
                        probe.least_squares_expected_minus_actual_over_manifest_rgb_gain
                    ),
    ...
    if !summary.focused_material_pixels.is_empty() {
        ...
        for focus in &summary.focused_material_pixels {
            ...
            out.push_str("| Pixel | Interpretation | Selected | Source | Browser material | Renderer material | RGBA A/E/S | Dist A-E / S-A / S-E | Frontmost | PBR terms | Nearest expected |\n");
            out.push_str("| --- | --- | --- | --- | --- | --- | --- | ---: | --- | --- | --- |\n");
...
fn texture_probe_summary(value: &Value) -> Result<TextureProbeSummary, Box<dyn Error>> {
    Ok(TextureProbeSummary {
        ...
        least_squares_expected_minus_actual_over_manifest_rgb_gain: optional_vec3_path(
            value,
            &["least_squares_expected_minus_actual_over_manifest_rgb_gain"],
        )?,
...
fn focused_material_pixel_row_summary(
    ...
    Ok(FocusedMaterialPixelRowSummary {
        ...
        .unwrap_or_else(|| "n/a".to_owned()),
        frontmost_pbr_terms: focused_pbr_terms_summary(value)?,
...
fn focused_pbr_terms_summary(value: &Value) -> Result<String, Box<dyn Error>> {
    let Some(terms) = optional_path(value, &["frontmost", "pbr_terms"]) else {
        return Ok("n/a".to_owned());
    };
    if terms.is_null() {
        return Ok("n/a".to_owned());
    ...
    Ok(format!(
        ...
        fmt_optional_vec3(optional_vec3_path(terms, &["specular_lobe_rgb"])?),
...
fn owner_render_pbr_term_output(
    ...
    Ok(OwnerRenderPbrTermOutputSummary {
        ...
        browser_total_srgb_sample_count: optional_u64_path(
            value,
            &["browser_total_srgb_sample_count"],
        )?
        .unwrap_or(0),
        mean_browser_total_srgb_actual_distance: optional_f64_path(
            value,
            &["mean_browser_total_srgb_actual_distance"],
        )?,
        mean_browser_total_srgb_expected_distance: optional_f64_path(
            value,
            &["mean_browser_total_srgb_expected_distance"],
        )?,
        browser_total_srgb_actual_closer: optional_u64_path(
            value,
            &["browser_total_srgb_actual_closer"],
        )?
        .unwrap_or(0),
        browser_total_srgb_expected_closer: optional_u64_path(
            value,
        ...
        browser_light_fit_sample_count: optional_u64_path(
            value,
            &["browser_light_fit_sample_count"],
        )?
        .unwrap_or(0),
        browser_actual_direct_scale_rgb: optional_vec3_path(
            value,
            &["browser_actual_direct_scale_rgb"],
        )?,
        browser_actual_ambient_scale_rgb: optional_vec3_path(
            value,
            &["browser_actual_ambient_scale_rgb"],
        )?,
        browser_expected_direct_scale_rgb: optional_vec3_path(
            value,
            &["browser_expected_direct_scale_rgb"],
        )?,
        browser_expected_ambient_scale_rgb: optional_vec3_path(
            value,
            &["browser_expected_ambient_scale_rgb"],
        )?,
        browser_light_fit_min_normalized_determinant: optional_f64_path(
            value,
            &["browser_light_fit_min_normalized_determinant"],
        )?,
        ...
        mean_rust_total_srgb_expected_distance: optional_f64_path(
            value,
            &["mean_rust_total_srgb_expected_distance"],
        )?,
        rust_total_srgb_actual_closer: optional_u64_path(
            value,
            &["rust_total_srgb_actual_closer"],
        )?
        .unwrap_or(0),
        rust_total_srgb_expected_closer: optional_u64_path(
            value,
            &["rust_total_srgb_expected_closer"],
        )?
        .unwrap_or(0),
        ...
        rust_light_fit_sample_count: optional_u64_path(value, &["rust_light_fit_sample_count"])?
            .unwrap_or(0),
        rust_actual_direct_scale_rgb: optional_vec3_path(value, &["rust_actual_direct_scale_rgb"])?,
        rust_actual_ambient_scale_rgb: optional_vec3_path(
            value,
            &["rust_actual_ambient_scale_rgb"],
        )?,
        rust_expected_direct_scale_rgb: optional_vec3_path(
            value,
            &["rust_expected_direct_scale_rgb"],
        )?,
        rust_expected_ambient_scale_rgb: optional_vec3_path(
            value,
            &["rust_expected_ambient_scale_rgb"],
        )?,
        rust_light_fit_min_normalized_determinant: optional_f64_path(
            value,
            &["rust_light_fit_min_normalized_determinant"],
        )?,
...
fn fmt_projection_distances(actual: Option<f64>, expected: Option<f64>, count: u64) -> String {
    match (actual, expected) {
        (Some(actual), Some(expected)) => format!(
            "{}/{} (n={count})",
            fmt_optional_pbr_distance(Some(actual)),
            fmt_optional_pbr_distance(Some(expected))
        ),
...
fn self_test() -> Result<(), Box<dyn Error>> {
    ...
    fs::write(
        ...
        r#"{
            ...
            "pbr_best_to_frontmost_term_output": {
                ...
                "browser_total_srgb_sample_count": 2,
                "mean_browser_total_srgb_actual_distance": 8.0,
                "mean_browser_total_srgb_expected_distance": 3.0,
                ...
                "browser_light_fit_sample_count": 2,
                "browser_actual_direct_scale_rgb": [1.0, 2.0, 3.0],
                "browser_actual_ambient_scale_rgb": [4.0, 5.0, 6.0],
                "browser_expected_direct_scale_rgb": [1.1, 2.1, 3.1],
                "browser_expected_ambient_scale_rgb": [4.1, 5.1, 6.1],
                "browser_light_fit_min_normalized_determinant": 0.25,
                ...
                "rust_light_fit_sample_count": 2,
                "rust_actual_direct_scale_rgb": [0.9, 1.9, 2.9],
                "rust_actual_ambient_scale_rgb": [3.9, 4.9, 5.9],
                "rust_expected_direct_scale_rgb": [0.8, 1.8, 2.8],
                "rust_expected_ambient_scale_rgb": [3.8, 4.8, 5.8],
                "rust_light_fit_min_normalized_determinant": 0.5,
    ...
    fs::write(
        ...
        r#"{
            ...
            "recommended_probes": [{
                ...
                "least_squares_expected_minus_actual_over_manifest_rgb_gain": [0.37, 0.46, 0.49],
                "manifest_sample_actual_within_1_5": 0,
    ...
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].browser_total_srgb_expected_closer,
        2
    );
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].mean_browser_total_srgb_actual_distance,
        Some(8.0)
    );
    assert_eq!(
    ...
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].browser_light_fit_sample_count,
        2
    );
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].browser_actual_direct_scale_rgb,
        Some([1.0, 2.0, 3.0])
    );
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].browser_expected_ambient_scale_rgb,
        Some([4.1, 5.1, 6.1])
    );
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].rust_actual_ambient_scale_rgb,
        Some([3.9, 4.9, 5.9])
    );
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0]
    ...
    assert_eq!(
        summary.owner_render_joins[0].pbr_term_outputs[0].rust_light_fit_min_normalized_determinant,
        Some(0.5)
    );
    ...
    assert!(summary_json.contains(r#""browser_actual_direct_scale_rgb":[1.0,2.0,3.0]"#));
    assert!(summary_json.contains(r#""browser_expected_ambient_scale_rgb":[4.1,5.1,6.1]"#));
    assert!(summary_json.contains(r#""rust_actual_ambient_scale_rgb":[3.9,4.9,5.9]"#));
    assert!(summary_json.contains(r#""browser_light_fit_min_normalized_determinant":0.25"#));
    assert!(summary_json.contains(
    ...
    assert!(markdown.contains("| backpack_nm | node145/mesh4/prim9/base | 15 | selected_sample_and_renderer_both_far | audit resolve draw binding or selected-surface material inputs | 35.8000 | 70.6000 / 106.2000 | 37.50,41.10,43.10 | 56.00,62.10,65.40 | 1.74,1.90,1.94 | 2.11,2.36,2.43 | 0.37,0.46,0.49 | 0/0/12 |"));
